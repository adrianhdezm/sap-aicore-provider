...
import { type ParseResult, safeParseJSON, extractResponseHeaders, type ResponseHandler, safeValidateTypes } from '@ai-sdk/provider-utils';
import { EventStreamCodec } from '@smithy/eventstream-codec';
import { toUtf8, fromUtf8 } from '@smithy/util-utf8';
import { ZodSchema } from 'zod';

// https://docs.aws.amazon.com/lexv2/latest/dg/event-stream-encoding.html
export const createConverseCompatibleEventStreamResponseHandler =
  <T>(chunkSchema: ZodSchema<T>): ResponseHandler<ReadableStream<ParseResult<T>>> =>
            ...
            while (buffer.length >= 4) {
              ...
              try {
                ...
                if (decoded.headers[':message-type']?.value === 'event') {
                  ...
                  let wrappedData = {
                    [decoded.headers[':event-type']?.value as string]: parsedDataResult.value
                  };
