...
const streamUrl = `${baseUrl}/converse-stream`;
const generateUrl = `${baseUrl}/converse`;
...
const model = new ConverseCompatibleChatLanguageModel(
  modelId,
  {},
  {
    provider: 'bedrock',
...
describe('doStream', () => {
  ...
  vi.mock('./converse-compatible-event-stream-response-handler', () => ({
    createConverseCompatibleEventStreamResponseHandler: (schema: any) => {
  ...
  function setupMockEventStreamHandler(options: { success?: boolean; errorValue?: any } = { success: true }) {
...
describe('doGenerate', () => {
  ...
  it('should include trace information in providerMetadata', async () => {
    ...
    const providerOptionsName = model.provider.split('.')[0]!.trim();
    expect(response.providerMetadata?.[providerOptionsName]!.trace).toMatchObject(mockTrace);
  });
