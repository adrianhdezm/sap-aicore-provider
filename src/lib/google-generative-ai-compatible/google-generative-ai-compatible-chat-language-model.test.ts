...
describe('doGenerate', () => {
  const TEST_URL_GEMINI_PRO = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent';

  const TEST_URL_GEMINI_2_0_PRO = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-pro:generateContent';

  const TEST_URL_GEMINI_2_0_FLASH_EXP = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-exp:generateContent';
  ...
  const prepareJsonResponse = ({
    ...
    groundingMetadata?: GoogleGenerativeAICompatibleGroundingMetadata;
    url?:
  ...
  describe('warnings for includeThoughts option', () => {
    it('should generate a warning if includeThoughts is true for a non-Vertex provider', async () => {
      ...
      const nonVertexModel = new GoogleGenerativeAICompatibleLanguageModel(
        'gemini-pro',
        {},
        {
    ...
    it('should NOT generate a warning if includeThoughts is true for a Vertex provider', async () => {
      ...
      const vertexModel = new GoogleGenerativeAICompatibleLanguageModel(
        'gemini-pro',
        {},
        {
      ...
      expect(warnings?.some((w) => w.type === 'other' && w.message.startsWith(expectedWarningMessage))).toBe(false);
    });
...
describe('doStream', () => {
  const TEST_URL_GEMINI_PRO = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:streamGenerateContent';

  const TEST_URL_GEMINI_2_0_PRO = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-pro:streamGenerateContent';
  ...
  it('should stream files', async () => {
    ...
    expect(events.filter((event) => event.type === 'file')).toEqual([{ type: 'file', mimeType: 'text/plain', data: 'test' }]);
  });
  ...
  it('should correctly stream reasoning parts and text deltas separately', async () => {
    ...
    const relevantEvents = events.filter((event) => event.type === 'text-delta' || event.type === 'reasoning');
    ...
    expect(finishEvent?.type === 'finish' && finishEvent.finishReason).toEqual('stop');
