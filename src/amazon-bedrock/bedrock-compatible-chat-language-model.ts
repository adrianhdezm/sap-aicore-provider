...
export class BedrockCompatibleChatLanguageModel implements LanguageModelV1 {
  ...
  private getArgs({
    ...
    const reasoningConfigOptions = BedrockReasoningConfigOptionsSchema.safeParse(providerMetadata?.bedrock?.reasoning_config);
  ...
  async doGenerate(options: Parameters<LanguageModelV1['doGenerate']>[0]): Promise<Awaited<ReturnType<LanguageModelV1['doGenerate']>>> {
    ...
    return {
      ...
      finishReason: mapBedrockFinishReason(response.stopReason as BedrockStopReason),
      usage: {
        ...
        new TransformStream<ParseResult<z.infer<typeof BedrockStreamSchema>>, LanguageModelV1StreamPart>({
          transform(chunk, controller) {
            ...
            if (value.messageStop) {
              finishReason = mapBedrockFinishReason(value.messageStop.stopReason as BedrockStopReason);
            }
