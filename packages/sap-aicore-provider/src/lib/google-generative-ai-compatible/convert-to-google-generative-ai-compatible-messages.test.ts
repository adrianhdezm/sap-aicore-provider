...
describe('system messages', () => {
  it('should store system message in system instruction', async () => {
    const result = convertToGoogleGenerativeAICompatibleMessages([{ role: 'system', content: 'Test' }]);
  ...
  it('should throw error when there was already a user message', async () => {
    expect(() =>
      convertToGoogleGenerativeAICompatibleMessages([
...
describe('user messages', () => {
  it('should add image parts for UInt8Array images', async () => {
    const result = convertToGoogleGenerativeAICompatibleMessages([
      {
        role: 'user',
        content: [
          {
